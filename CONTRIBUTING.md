...
- Type-check with `mypy` (this repo runs `mypy --strict`).
- Keep functions documented with docstrings.

## Diamond Interface packages

This repository implements four Diamond Interface packages — `theta_resonance`,
`epi_sigillin`, `hikari_ledger`, and `diffusive_routing` — each exposing
`run_cycle`, `get_crep_state`, `get_utac_state`, `get_phase_events`, and
