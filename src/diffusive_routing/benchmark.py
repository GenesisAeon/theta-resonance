...
def run_benchmark(
    ...
    results: dict[str, object] = {
        "throughput_vs_ospf_ratio": _check(
            throughput_vs_ospf, *t["throughput_vs_ospf_ratio"], "throughput_vs_ospf_ratio"
        ),
        "latency_vs_shortest_path": _check(
            latency_vs_sp, *t["latency_vs_shortest_path"], "latency_vs_shortest_path"
        ),
        "gamma_routing": _check(
            gamma_routing, *t["gamma_routing"], "gamma_routing"
        ),
        "congestion_events_per_hour": _check(
            congestion_events_per_hour,
            *t["congestion_events_per_hour"],
            "congestion_events_per_hour",
        ),
        "load_balance_gini": _check(
            load_balance_gini, *t["load_balance_gini"], "load_balance_gini"
        ),
        "adaptation_time_ms": _check(
            adaptation_time_ms, *t["adaptation_time_ms"], "adaptation_time_ms"
        ),
    }

    passes = [
        v["pass"]  # type: ignore[index]
        for v in results.values()
        if isinstance(v, dict) and v["pass"] is not None  # type: ignore[index]
    ]
